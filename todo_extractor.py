...
def grep_cmd(search_string):
    ...
    """
    cmd = ['grep', '-rn']
    if not subprocess.run('command -v rg', shell = True, stdout =subprocess.DEVNULL,stderr=subprocess.DEVNULL).returncode:
        cmd = ['rg', '-n']
...
def search_folder(folder):
    ...
    r = subprocess.run(grep_cmd(TODO_REGEXP), cwd=folder, stdout =   subprocess.PIPE,stderr=subprocess.PIPE, encoding='utf-8')
    return [m.groupdict() for m in re.finditer(r'^(?P<file>.*):(?P<line>\d*):.*TODO:\s*(?P<comment>.*)', r.stdout, re.M)]
